...
;; Author:      Robert Fenk
;; Status:      Tested with XEmacs 21.4.19 & GNU Emacs 21.4.1 & VM 7.19
;; Keywords:    extensions, vm
;; X-URL:       http://www.robf.de/Hacking/elisp
;; X-RCS:       $Id: esmtpmail.el,v 1.16 2007-01-17 23:05:24 widhopf Exp $
...
AUTHENTICATION-METHOD and METHOD-DATA have to be one of
    helo
    ehlo
    login USER [PASSWORD]
  ...
  (let ((process nil)
    ...
    (if esmtpmail-debug-info
        (message "ESMTP: from <%s> to %s%s" from host
                 (if auth-data
                     (format " with %S <%S>" auth-fun auth-data)
                   "")))
...
(defun esmtpmail-send-command (process command &optional omit)
  (goto-char (point-max))
  (if omit
      (insert omit "\r\n")
    (insert command "\r\n"))
...
(defun esmtpmail-authentication-by-login
  ...
  (let ((login-and-pw (esmtpmail-get-login-and-password host login password
    ...
    (esmtpmail-check-for-error process)
    (let ((code (catch 'done
                  (esmtpmail-send-command process elogin "[Login omitted]")
                  (esmtpmail-check-for-error process)
                  (esmtpmail-send-command process password "[Password omitted]")
                  (esmtpmail-check-for-error process)
                  t)))
...
(defun esmtpmail-authentication-by-vm-pop-login
  ...
  (let* ((source-list (vm-parse spoolname "\\([^:]+\\):?"))
    ...
    (if (null password)
        (setq password (read-passwd (format "POP password for %s: "
					      (nth 0 source-list)))
